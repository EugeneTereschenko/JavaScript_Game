window.addEventListener('DOMContentLoaded', function(){
    const canavas = document.getElementById('canvas1');
    ...
    class Player {
        ...
        update(input, deltaTime, enemies){
            ...
            //else if (input.keys.includes('ArrowDown')) {this.vy = 5;}
            else {
                this.speed = 0;
                this.vy = 0;
            }
    ...
    function handleEnemies(deltaTime) {
        if (enemyTimer > enemyInterval + randomEnemyInterval) {
            enemies.push(new Enemy(canavas.width, canavas.height));
            enemyTimer = 0;
    ...
    function displayStatusText(context) {
        ...
        if (gameOver) {
            ...
            context.fillStyle = 'black';
            context.fillText('GAME OVER press Enter or swipe up to restart: ', canavas.width / 2, canavas.height / 2);
            context.fillStyle = 'white';
            context.fillText('GAME OVER press Enter or swipe up to restart: ', canavas.width / 2 + 2, canavas.height / 2 + 2);
        }
    }
    ...
    function toggleFullScreen() {
        if (!document.fullscreenElement) {
            canavas.requestFullscreen().catch(err => {
    ...
    const player = new Player(canavas.width, canavas.height);
    const background = new Background(canavas.width, canavas.height);
